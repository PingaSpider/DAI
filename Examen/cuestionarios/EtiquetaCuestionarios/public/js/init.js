...
function addPregunta(event){
    ...
    if(pregunta){
        ...
        if(trueOfalses === null){
            ...
            enviarPreguntaAlServidor(cuestionarioId,pregunta,trueOfalses.value);
...
//COMPONER PREGUNTA
function componedorPregunta(cuestionarioId,preguntaId,pregunta,respuesta){
        ...
        if(data && data.result){
            data.result.forEach(pregunta => {
                if(pregunta.preguntaID && pregunta.pregunta && pregunta.respuesta !== undefined){
                    componedorPregunta(cuestionarioId,pregunta.preguntaID,pregunta.pregunta,pregunta.respuesta);
                }else{
...
function enviarPreguntaAlServidor(cuestionarioId, pregunta, respuesta) {
    const url = `${base}/cuestionarios/${cuestionarioId}/preguntas`;
    ...
    fetch(url, request)
        ...
        .then(data => {
            if (data && data.result) {
                componedorPregunta(cuestionarioId, data.result.preguntaID, pregunta, respuesta);
...
function addCuestionario(event){
    ...
    if(ciudad){
        ...
        enviarCuestionarioAlServidor(ciudadA,etiquetaColor);
...
function componerCuestionario(ciudad,cuestionarioId,etiquetaColor){
    ...
    opcion1.textContent  = "Amarillo";

    const opcion2 = document.createElement('option');
    opcion2.textContent  = "Rojo";

    const opcion3 = document.createElement('option');
    opcion3.textContent  = "Verde";

    const mensajeOpcion = document.createElement('option');
    mensajeOpcion.innerText = "-Selecciona una Opcion-";
    mensajeOpcion.toggleAttribute("disabled");
    mensajeOpcion.toggleAttribute("selected");

    selectEtiqueta.appendChild(mensajeOpcion);
    selectEtiqueta.appendChild(opcion1);
    selectEtiqueta.appendChild(opcion2);
    selectEtiqueta.appendChild(opcion3);

    const divColor = document.createElement('div');
    divColor.innerHTML = "&#x25AB";
    divColor.className = etiquetaColor;

    section.appendChild(divColor);

    selectEtiqueta.addEventListener('click',function(e){
        const opcionSeleccionada = e.target.closest('option');
        const color = opcionSeleccionada.textContent;
        if(color){
            if(color === "Amarillo"){
                divColor.className = "Amarillo";
                actualizarCuestionario(cuestionarioId,"Amarillo");
            }else if(color === "Rojo"){
                divColor.className = "Rojo";
                actualizarCuestionario(cuestionarioId,"Rojo");
            }else{
                divColor.className = "Verde";
                actualizarCuestionario(cuestionarioId,"Azul");
            }
        }
    })

    section.appendChild(selectEtiqueta);
    section.appendChild(divColor);
    ...
    .then(data => {
        if(data && data.result){
            data.result.forEach(cuestionario => {
                componerCuestionario(cuestionario.tema,cuestionario.cuestionarioID,cuestionario.etiquetaColor);
...
function enviarCuestionarioAlServidor(tema,etiquetaColor){
    const url = `${base}/cuestionarios`;
    const request = {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if(data && data.result){
            componerCuestionario(tema,data.result.cuestionarioID,etiquetaColor);
            // Limpiar el input después de crear exitosamente
...
function actualizarCuestionario(cuestionarioId,etiquetaColor){
    const url = `${base}/cuestionarios/${cuestionarioId}`;
    const request = {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
    ...
    .then(response => {
        if(response.ok){
            window.alert(`Cuestionario ${cuestionarioId} actualizado a color ${etiquetaColor}`);
        }
    })
    .catch(error =>{
        console.error('Error:', error);
