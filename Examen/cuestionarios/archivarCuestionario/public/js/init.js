...
function addPregunta(event){
    ...
    //ver si el cuestionario esta archivado 
    const estaArchivado = sectionInsert.getAttribute('data-archivado');

    //Validar que la pregunta no este vacia
    if(estaArchivado === "false"){
        if(pregunta){
        ...
        window.alert("Los Cuestionarios Archivados no pueden agregar preguntas");
    }
...
function addCuestionario(event){
    ...
    if(ciudad){
        ...
        enviarCuestionarioAlServidor(ciudadA,archivado);
...
function componerCuestionario(ciudad,cuestionarioId,archivado){
    ...
    //Agregar el boton de archivado
    const buttonArchivado = document.createElement('input');
    buttonArchivado.type = "button";
    buttonArchivado.className = "btn-archivo";
    section.appendChild(buttonArchivado);
    ...
    if(archivado === "true"){
        section.setAttribute('data-archivado',"true");
        buttonArchivado.value = "Desarchivar";
    }else{
        section.setAttribute('data-archivado',"false");
        buttonArchivado.value = "Archivar";
    }

    buttonArchivado.addEventListener('click',function(){
        const estaArchivado = section.getAttribute("data-archivado");
        if(estaArchivado === "false"){
            buttonArchivado.value = "Desarchivar";
            section.removeAttribute('data-archivado');
            section.setAttribute('data-archivado',"true");
            actualizarEstadoCuestionario(section.id,"true");

        }else{
            section.removeAttribute('data-archivado');
            section.setAttribute('data-archivado',"false");
            actualizarEstadoCuestionario(section.id,"false");
            buttonArchivado.value = "Archivar";
        }
    });
    ...
    .then(data => {
        if(data && data.result){
            data.result.forEach(cuestionario => {
                componerCuestionario(cuestionario.tema,cuestionario.cuestionarioID,cuestionario.archivado);
...
function enviarCuestionarioAlServidor(tema,archivado){
    const url = `${base}/cuestionarios`;
    const request = {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if(data && data.result){
            componerCuestionario(tema,data.result.cuestionarioID,archivado);
            // Limpiar el input después de crear exitosamente
...
function actualizarEstadoCuestionario(cuestionarioId,archivado){
    ...
    .then(response => {
        if(response.ok){
           window.alert("El cuestionario ha sido archivado/desarchivado")
        } else {
            return response.json().then(data => {
