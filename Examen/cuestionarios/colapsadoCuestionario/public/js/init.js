...
//LLAMAR A ACTUALIZAR COLPASADO
document.addEventListener('click',(event)=>{
    const detailsButton = event.target.closest('details');

    if(detailsButton){
        var colapsado = "false";
        const section = detailsButton.closest('section');
        if(detailsButton.open){
            colapsado = "true";
        }
        actualizarColapsado(section.id,colapsado);
    }
})
...
function componedorPregunta(cuestionarioId,preguntaId,pregunta,respuesta){
    ...
    const numeroActual =  parseInt(detailsSection.getAttribute("numeropreguntas"));
    detailsSection.setAttribute("numeropreguntas",numeroActual +1);
    const summary = detailsSection.querySelector("summary");
    if(summary){
        summary.textContent = `Preguntas del cuestionario: (${detailsSection.getAttribute("numeropreguntas")})`;
    }
...
function borrarPregunta(bloqueNode){
    ...
    .then(response => {
        if(response.ok){
            ...
            pregunta.remove();
            const  detailsSection = cuestionario.querySelector("details");
            const numeroActual =  parseInt(detailsSection.getAttribute("numeropreguntas"));
            detailsSection.setAttribute("numeropreguntas",numeroActual -1);
            const summary = detailsSection.querySelector("summary");
            if(summary){
...
function addCuestionario(event){
    ...
    if(ciudad){
        ...
        const colapsado = "false"; //Por defecto el cuestionario no esta colapsado 
        enviarCuestionarioAlServidor(ciudadA,colapsado);
...
function componerCuestionario(ciudad,cuestionarioId,colapsado){
    
    const details = document.createElement("details");
    details.setAttribute("numeropreguntas", 0);
    const summary = document.createElement("summary");
    summary.textContent = "Preguntas del formulario";
    ...
    .then(data => {
        if(data && data.result){
            data.result.forEach(cuestionario => {
                componerCuestionario(cuestionario.tema,cuestionario.cuestionarioID,cuestionario.colapsado);
...
function actualizarColapsado(cuestionarioId,colapsado){
    const url = `${base}/cuestionarios/${cuestionarioId}`; // Cambiado a plural "cuestionarios"
    const request =  {
        method: 'PUT',
        headers: {
    ...
    .then(response => {
        if(response.ok){
            console.log("colapsado actualizado");
        } else {
            return response.json().then(data => {
                throw new Error(data.error || 'Error al actualizar colapsado del cuestionario');
            });
        }
    })
    .catch(error => {
...
function enviarCuestionarioAlServidor(tema,colapsado){
    const url = `${base}/cuestionarios`;
    const request = {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if(data && data.result){
            componerCuestionario(tema,data.result.cuestionarioID,colapsado);
            // Limpiar el input después de crear exitosamente
