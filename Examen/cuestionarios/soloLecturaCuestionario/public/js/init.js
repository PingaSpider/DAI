...
function addPregunta(event){
    ...
    if(sectionInsert.getAttribute("data-soloLectura") === "true"){
        window.alert("Los cuestionarios de solo lectura no se pueden modificar");
        return;
    }
...
function borrarPregunta(bloqueNode){
    ...
    if(cuestionario.getAttribute("data-soloLectura") === "true"){
        window.alert("Los cuestionarios de solo lectura no se pueden modificar");
        return;
    }
    fetch(url,{
...
function addCuestionario(event){
    ...
    if(ciudad){
        ...
        enviarCuestionarioAlServidor(ciudadA,soloLectura);
...
function componerCuestionario(ciudad,cuestionarioId,soloLectura){

    const section = document.createElement('section');
    section.id = cuestionarioId;

    const divSoloLectura = document.createElement('div');
    const checkBox = document.createElement('input');
    checkBox.type = "checkbox";


    divSoloLectura.textContent = "Solo lectura";
    section.appendChild(divSoloLectura);
    divSoloLectura.appendChild(checkBox);

    if(soloLectura === 1){
        section.setAttribute("data-soloLectura", "true");
        checkBox.toggleAttribute("checked","true");
    }else{
        section.setAttribute("data-soloLectura", "false");
    }


    checkBox.addEventListener("click",function(e){
        const esOnlyLectura = checkBox.checked

        if(esOnlyLectura){
            section.removeAttribute("data-soloLectura");
            section.setAttribute("data-soloLectura","true");
            actualizarEstadoCuestionario(cuestionarioId,esOnlyLectura)
        }else{
            section.removeAttribute("data-soloLectura");
            section.setAttribute("data-soloLectura","false");
            actualizarEstadoCuestionario(cuestionarioId,esOnlyLectura)
    ...
    .then(data => {
        if(data && data.result){
            data.result.forEach(cuestionario => {
                componerCuestionario(cuestionario.tema,cuestionario.cuestionarioID,cuestionario.soloLectura);
...
function enviarCuestionarioAlServidor(tema,soloLectura){
    const url = `${base}/cuestionarios`;
    const request = {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if(data && data.result){
            componerCuestionario(tema,data.result.cuestionarioID,soloLectura);
            // Limpiar el input después de crear exitosamente
...
function actualizarEstadoCuestionario(cuestionarioId,soloLectura){
    ...
    .then(response => {
        if(response.ok){
            console.log(`Cuestionario ${cuestionarioId} ha sido actualizado su estado de lectura/escritura`);
        }else{
            //Lanzar error
            throw new Error('Error al enviar la solicitud de actualizacion de cuestionario al servidor');
        }
    })
    .catch(error => {
        console.error('Error:', error);
