...
function addPregunta(event){
    ...
    if(pregunta){
        ...
        if(trueOfalses === null){
            ...
            enviarPreguntaAlServidor(cuestionarioId,pregunta,trueOfalses.value,esFavorito);
...
//COMPONER PREGUNTA
function componedorPregunta(cuestionarioId,preguntaId,pregunta,respuesta,esFavorito){
    

    const div = document.createElement('div');
    div.className = 'bloque';
    div.setAttribute("data-favorito",esFavorito);
    ...
    estrellaDiv.innerHTML = "&#9734";
    estrellaDiv.className = "estrella";
    ...
    estrellaDiv.addEventListener('click',function(){
        const favoritoDIV = div.getAttribute("data-favorito");
        if(favoritoDIV === "true"){
            div.removeAttribute("data-favorito");
            div.setAttribute("data-favorito","false");
            actualizarFavorito(cuestionarioId,preguntaId,"false");
            estrellaDiv.classList.toggle("red");
        }else{
            const preguntas = section.querySelectorAll('.bloque');
            var hayFavorito = 0;

            preguntas.forEach(preguntax =>{
                if(preguntax.getAttribute("data-favorito") === "true"){
                    hayFavorito += 1;
                }
            })
            ...
            if(hayFavorito === 0){
                div.removeAttribute("data-favorito");
                div.setAttribute("data-favorito","true");
                actualizarFavorito(cuestionarioId,preguntaId,"true");
                estrellaDiv.classList.toggle("red");
            }else{
                window.alert("No pueden haber dos preguntas favoritas");
            }
        }
    });

    if(esFavorito === "true"){
        estrellaDiv.classList.add("red");
    }
...
function borrarPregunta(bloqueNode){
    ...
    const preguntas = cuestionario.querySelectorAll('.bloque');

    if(preguntas.length > 1 && pregunta.getAttribute("data-favorito") === "true"){
        window.alert("Las preguntas favoritas no se pueden borrar, a menos que sean la ultima");
        return;
    }
    fetch(url,{
        method: 'DELETE',
        ...
        if(data && data.result){
            data.result.forEach(pregunta => {
                if(pregunta.preguntaID && pregunta.pregunta && pregunta.respuesta !== undefined){
                    componedorPregunta(cuestionarioId,pregunta.preguntaID,pregunta.pregunta,pregunta.respuesta,pregunta.esFavorito);
                }else{
...
function enviarPreguntaAlServidor(cuestionarioId, pregunta, respuesta,esFavorito) {
    const url = `${base}/cuestionarios/${cuestionarioId}/preguntas`;
    ...
    fetch(url, request)
        ...
        .then(data => {
            if (data && data.result) {
                componedorPregunta(cuestionarioId, data.result.preguntaID, pregunta, respuesta,esFavorito);
...
function actualizarFavorito(cuestionarioId,preguntaId,esFavorito){
    const url = `${base}/cuestionarios/${cuestionarioId}/preguntas/${preguntaId}`;
    const request = {
        method: 'PUT',
        headers: {
    ...
    fetch(url, request)
        .then(response => {
            if (!response.ok) {
                if(response.status === 409){
                    window.alert("No se pudo actualizar el estado de favorito de la pregunta")
                }
                else{
                    window.alert(`Se ha actualizado el estado de favorito de la pregunta ${preguntaId}` );
                }
            }
            return response.json();
        })
