...
function addPregunta(event){
    ...
    if(pregunta){
        ...
        if(trueOfalses === null){
            ...
            enviarPreguntaAlServidor(cuestionarioId,pregunta,trueOfalses.value,preferido);
...
//COMPONER PREGUNTA
function componedorPregunta(cuestionarioId,preguntaId,pregunta,respuesta,preferido){
    ...
    const preferida = document.createElement('div');
    preferida.textContent = "Pregunta Fijada";

    const checkedBox = document.createElement('input');
    checkedBox.type = "checkbox";

    if(preferido === "true"){
        checkedBox.toggleAttribute("checked","true");
        div.classList.toggle("red");
    }

    preferida.appendChild(checkedBox);
    preferida.className = "preferido";

    checkedBox.addEventListener('click',function(e){
        const esFav = checkedBox.checked;
        if(esFav === false){
            div.classList.toggle("red");
            preferidaPregunta(div,"false");
        }else{
            const preguntas = section.querySelectorAll('.bloque');
            var hayPreferida = 0;
            preguntas.forEach(preguntax =>{
                if(preguntax.classList.length == 2){
                    hayPreferida += 1;
                }
            })
            if(hayPreferida > 0){
...
function borrarPregunta(bloqueNode){
    ...
    const cantidadPreguntas = cuestionario.querySelectorAll('.bloque').length;
    const esPreferida = pregunta.classList.contains("red");
    if(esPreferida && cantidadPreguntas > 1 ){
        window.alert("Una pregunta favorita no se puede borrar, a menos que sea la última");
        return;
    }
        ...
        if(data && data.result){
            data.result.forEach(pregunta => {
                if(pregunta.preguntaID && pregunta.pregunta && pregunta.respuesta !== undefined){
                    componedorPregunta(cuestionarioId,pregunta.preguntaID,pregunta.pregunta,pregunta.respuesta,pregunta.preferido);
                }else{
...
function enviarPreguntaAlServidor(cuestionarioId, pregunta, respuesta,preferido) {
    const url = `${base}/cuestionarios/${cuestionarioId}/preguntas`;
    ...
    fetch(url, request)
        ...
        .then(data => {
            if (data && data.result) {
                componedorPregunta(cuestionarioId, data.result.preguntaID, pregunta, respuesta,preferido);
...
//Actualizar Pregunta
function preferidaPregunta(bloqueNode,preferida){
    const pregunta = bloqueNode.closest('.bloque');
    const cuestionario = bloqueNode.closest('section');
    const cuestionarioId = cuestionario.id;
    const preguntaId = pregunta.dataset.preguntaId;
    ...
    .then(response => {
        if(response.ok){
            console.log("Actualizada la pregunta");
        }
        else{
